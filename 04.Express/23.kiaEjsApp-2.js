...
const dm = require('./db/kia-module');
...
app.get('/', (req, res) => {
    dm.getList(rows => {
        ejs.renderFile('views/23.index-2.ejs',
                        {rows},                 // {rows:rows}            
                        (err,html)=>{
            res.send(html);
        });
...
app.get('/create', (req, res) => {
    ejs.renderFile('views/23.create-2.ejs',(err,html)=>{
        res.send(html);
    });
});
...
app.get('/update/:id', (req, res) => {  // http://localhost:3000/update?id=123
    ...
    dm.getPlayer(id, rows => {
        ...
        ejs.renderFile('views/23.update-2.ejs',{
...
app.get('/delete/:id', (req, res) => {      // http://localhost/delete?id=123
    const id = parseInt(req.params.id);
    ejs.renderFile('views/23.delete-2.ejs',{
        id
    },(err,html)=>{
        res.send(html)
    });
